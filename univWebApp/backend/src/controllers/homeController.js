...
const getOneItem= asyncHandler(async (req, res) => {
	try {
		if (!mongoose.Types.ObjectId.isValid(req.params.id))
			return res.status(400).json({message: 'Id format is not valid'});
...
const createOneItem = asyncHandler(async (req, res) => {
	const { type, description, status, room, createdAt } = req.body;
	try {
...
const getOneUser = asyncHandler(async (req, res) => {
	try {
		if (!mongoose.Types.ObjectId.isValid)
			return res.status(404).json({message: "Id format is not valid"});
...
const requestOneItem = asyncHandler(async (req, res) => {
	...
	if (item)
	{
		...
		await item.save();
		res.status(200).json({ message: `Item requested successfully by ${req.body.userId}` });
	}
	else
		res.status(404).json({ message: 'Item requested not found' });
});
...
const approve = asyncHandler(async (req, res) => {
	...
	if (item)
	{
		...
		await item.save();
		res.status(200).json({ message: `Item request ${status}` });
	} else {
...
const returnOneItem = asyncHandler(async (req, res) => {
	...
	if (item)
	{
		...
		await item.save();
		res.status(200).json({ message: 'Item was returned' });
	}
	else
	{
		res.status(404).json({ message: 'Server hanging' });
	}
});
